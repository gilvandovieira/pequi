...
1. `deno fmt --check`
2. `deno task lint`
3. `deno task doc:coverage` — **documentation gate** (also enforced locally by the
   `.githooks/pre-commit` hook; run `deno task hooks:install` once to enable it). Requires 100%
   module-doc coverage and ≥80% exported-symbol JSDoc coverage across the library source.
4. `deno task check`
5. `deno task test` — source tests plus bundle semantic-equivalence, type-export, and import smoke
   tests. The pure suite runs without `--allow-ffi`, so native tests skip cleanly here.
6. `deno task test:native` — **native tests where available**. The ubuntu-x86_64 runner ships the
   committed prebuilt `.so`, so the Rust sink loads via FFI and its lifecycle/equivalence tests run
   for real.
7. `deno task native:verify:artifacts` — **artifact shape gate** (not a runtime test). Confirms each
...
8. `deno task bundle:verify` — the **freshness gate**.
9. `deno task bench:smoke` — **benchmark smoke**. Runs every bundle/source/native/Pino variant at a
